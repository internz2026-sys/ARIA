...
const navItems = [
  ...
  },
  {
    ...
    // Stagnation Monitor pulse — count of inbox drafts >24h old that
    // haven't been actioned. Surfaces buried tasks even when the user
    // is focused on newer work in another section of the app.
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714a2.25 2.25 0 00.659 1.591L19 14.5M14.25 3.104c.251.023.501.05.75.082M19 14.5l-1.341 4.023A2.25 2.25 0 0115.523 20H8.477a2.25 2.25 0 01-2.136-1.477L5 14.5m14 0H5" />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5m.75-9l3-3 2.148 2.148A12.061 12.061 0 0116.5 7.605" />
      </svg>
    ),
  },
  {
...
export default function Sidebar() {
  ...
  return (
    <aside className="h-full bg-white border-r border-[#E0DED8] flex flex-col">
      {/* Logo */}
      ...
      <div className="p-4 border-t border-[#E0DED8]">
        <div className="flex items-center gap-3 px-2">
          <div className="w-8 h-8 rounded-full bg-[#EEEDFE] flex items-center justify-center">
            <span className="text-[#534AB7] text-xs font-semibold">{user?.initials || "--"}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-[#2C2C2A] truncate">{user?.name || "Loading..."}</p>
            <p className="text-xs text-[#5F5E5A] truncate">{user?.email || ""}</p>
          </div>
          <button
            onClick={handleSignOut}
            title="Sign out"
            className="text-[#5F5E5A] hover:text-[#D85A30] transition-colors"
          >

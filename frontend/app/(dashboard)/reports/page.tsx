...
/** Tiny markdown -> HTML for the body. The reports body is always
 *  generated by Haiku (or a deterministic fallback) and is constrained
 *  to short paragraphs + simple lists, so we don't need a full mdx
 *  pipeline. Bold + italics + bullets cover the format used by both
 *  paths. */
function renderMarkdownBody(md: string): string {
  ...
  // Wrap consecutive `- foo` lines into a <ul>
  html = html.replace(/(?:^|\n)((?:-\s.+\n?)+)/g, (match) => {
    const items = match
      .trim()
      .split("\n")
  ...
  html = html
    .split(/\n{2,}/)
    .map((para) => (para.trim().startsWith("<ul>") ? para : `<p>${para.replace(/\n/g, "<br/>")}</p>`))
    .join("\n");
  return html;
}
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl border border-[#E0DED8] shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {report.chart_urls.map((c, i) => (
            <figure key={i} className="bg-white border border-[#E0DED8] rounded-lg overflow-hidden">
          ...
          {report.body_markdown && (
            <div
              className="prose prose-sm max-w-none text-[#2C2C2A] [&>p]:my-3 [&>ul]:my-3 [&>ul]:pl-6 [&>ul]:list-disc [&>ul>li]:my-1 [&_strong]:font-semibold [&_code]:bg-[#F0EFEC] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_code]:text-[12px]"
              dangerouslySetInnerHTML={{ __html: renderMarkdownBody(report.body_markdown) }}
            />
          )}

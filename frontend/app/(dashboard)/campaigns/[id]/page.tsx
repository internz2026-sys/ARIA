...
import CampaignPerformanceForm from "@/components/shared/CampaignPerformanceForm";
import CampaignStatusSelector from "@/components/shared/CampaignStatusSelector";
...
export default function CampaignDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "copypaste" | "performance" | "ai" | "history">("overview");
  ...
  // Performance metrics block ('Reported Performance') is stored in
  // metadata.performance via Coder 2's update_campaign_metrics helper.
  const performance: any = campaign?.metadata?.performance || null;
  // Impressions hint for CTR auto-compute — pulled from the latest
  // uploaded report's totals if available.
  const impressionsHint: number | null =
    metrics?.impressions != null ? Number(metrics.impressions) : null;
      ...
      <div className="border-b border-[#E0DED8]">
        <div className="flex gap-6 overflow-x-auto">
          {(() => {
            ...
            // pure-draft campaigns (would just show an empty form
            // before the user has anything to report).
            if (campaign?.status === "active" || campaign?.metadata?.performance) {
              tabs.push({ key: "performance", label: "Performance" });
            }
      ...
      {activeTab === "performance" && (
        <div className="space-y-4">
          <CampaignPerformanceForm
            ...
            campaignId={id}
            initial={performance}
            impressionsHint={impressionsHint}
            onSaved={load}
          />
        </div>
      )}
              ...
              <div className="bg-white rounded-xl border border-[#E0DED8] p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => downloadReportPdf(latestReport.ai_report_text, latestReport.ai_recommendations, campaign.campaign_name)}

...
export default function ResetPasswordRequestPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(email.trim().toLowerCase(), {
      // Supabase will redirect the user here after they click the
      // reset link in their email; the page reads the recovery token
      // out of the URL fragment automatically.
      ...
      <div className="bg-white rounded-2xl border border-[#E0DED8] shadow-sm p-6 sm:p-8">
        {!sent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full h-12 rounded-lg border px-3 text-[15px] text-[#2C2C2A] placeholder:text-[#B0AFA8] outline-none transition focus:ring-2 focus:ring-[#534AB7]/20 focus:border-[#534AB7] ${error ? "border-[#D85A30]" : "border-[#E0DED8]"}`}
                />
          ...
          <>
            ...
            <h1 className="text-[24px] font-bold text-[#2C2C2A] text-center mb-2">Check your email for a reset link</h1>
            ...
            <p className="text-xs text-[#5F5E5A] text-center mb-6">
              ...
              <button onClick={() => setSent(false)} className="text-[#534AB7] font-medium hover:underline">
                try again
              </button>
            </p>
          </>

...
async def send_gmail_email(tenant_id: str, body: GmailSendRequest, confirmed: bool = False):
    ...
    result = await _gmail_send_with_refresh(
        tenant_id,
        to=body.to,
        subject=body.subject,
...
async def approve_and_send_email(tenant_id: str, body: EmailApproveRequest):
    ...
    # Mark as sending. If ANY downstream step fails, we flip this to
    # "failed" in the exception handler below so the inbox row doesn't
    # get stuck in "sending" forever.
    sb.table("inbox_items").update({
        "status": "sending",
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }).eq("id", body.inbox_item_id).execute()
    ...
    # pull the Gmail threadId + newest inbound Message-ID so Gmail AND
    # third-party mail clients thread the reply correctly. Best-effort —
    # a missing lookup just starts a fresh Gmail conversation.
    config = get_tenant_config(tenant_id)
    reply_thread_id = ""
    reply_in_reply_to = ""
    reply_to_thread_db_id = meta.get("reply_to_thread_id", "")
    if reply_to_thread_db_id:
        reply_thread_id, reply_in_reply_to = await _resolve_reply_thread_context(
...
async def send_thread_reply(tenant_id: str, thread_id: str, body: SendReplyRequest):
    ...
    html_body = _wrap_html(_user_text_to_html(text))

    config = get_tenant_config(tenant_id)
    gmail_thread_id, in_reply_to = await _resolve_reply_thread_context(
        tenant_id, thread_id,
        access_token=config.integrations.google_access_token or "",
    )

...
async def _execute_delegation(
    ...
    # Pipeline image hint: when the chain had a media step earlier, append
    # an explicit "include the recent generated image" sentence so the
    # downstream agent's existing image-keyword detector fires and pulls
    # the fresh asset. Skip if the task already mentions image vocabulary
    # (don't double-prompt). Runs for BOTH paperclip and local dispatch
    # since the hint lives in the task text, not a Python context dict.
